...
public:
    ...
    //Задание
    void UpdateAccessCounter(Node* node) const;

    // Новый метод для поиска самого популярного слова
    Node* GetMostPopularWord() const;

    // Новый метод для поиска самого непопулярного слова
    Node* GetLeastPopularWord() const;
};